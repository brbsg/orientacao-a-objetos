...
public class PersonInformationScreen implements ActionListener {
	...
	public void inserirEditar(int op, DataController d, PersonScreen p, int pos) {
		...
		if (op == 3) {
			valorNome = new JTextField(dados.getClients()[pos].getName(), 200);
			valorEnd = new JTextField(dados.getClients()[pos].getEndereco(), 200);
			valorDataC = new JTextField(dados.getClients()[pos].getDataPrimC(), 200);
			valorsalario = new JTextField(200);
			valorCPF = new JTextField(dados.getClients()[pos].getCPF(), 200);
			valorID = new JTextField(String.valueOf(dados.getClients()[pos].getNumID()), 200);
			valorDDD = new JTextField(String.valueOf(dados.getClients()[pos].getNumTel().getDDD()), 3);
			valorTelefone = new JTextField(String.valueOf(dados.getClients()[pos].getNumTel().getNumero()), 10);

		} else if (op == 4) { // Preenche dados com dados do funcionario clicado
			valorNome = new JTextField(dados.getEmployees()[pos].getName(), 200);
			valorsalario = new JTextField(String.valueOf(dados.getEmployees()[pos].getSalary()), 200);
			valorDataC = new JTextField(200);
			valorEnd = new JTextField(dados.getEmployees()[pos].getAddress(), 200);
			valorCPF = new JTextField(dados.getEmployees()[pos].getCPF(), 200);
			valorID = new JTextField(String.valueOf(dados.getEmployees()[pos].getNumID()), 200);
			valorDDD = new JTextField(String.valueOf(dados.getEmployees()[pos].getNumTel().getDDD()), 3);
			valorTelefone = new JTextField(String.valueOf(dados.getEmployees()[pos].getNumTel().getNumero()), 10);
		...
		if (src == botaoSalvar) {
			try {
				...
				if (opcao == 1) // cadastro de novo cliente
					novoDado[0] = Integer.toString(dados.getAmountOfClients());
				else if (opcao == 2) // cadastro de novo funcionario
					novoDado[0] = Integer.toString(dados.getAmountOfEmployees());
